...
Here we do not need to recalculate all the scores, but just the goal-specific resilience (these will be compared to status).To do this, I will rerun the whole process, but rename `resilience_matrix_reg.csv` -> `resilience_matrix.csv`.  Then all scores except resilience will be deleted, and the new .csv saved.  Repeat this for `resilience_matrix_soc.csv` and `resilience_matrix_ecol.csv`.

Note also the social pressure needs to be removed from the pressures marix to avoid collinearity with the social resilience layer (which is the inverse of the pressure).  This is performed in the first block.


## Regulatory resilience

```{r reg_resilience}
# replace resilience_matrix.csv with resilience_matrix_reg.csv
file.copy(from = 'conf/resilience_matrix_reg.csv',
          to   ='conf/resilience_matrix.csv',
          overwrite = TRUE)
file.copy(from = 'conf/pressures_matrix_no_soc.csv',
          to   = 'conf/pressures_matrix.csv',
          overwrite = TRUE)
...
prs_scores_all <- scores_all_years %>%
  filter(dimension == 'pressures')
res_scores_all <- scores_all_years %>%
  filter(dimension == 'resilience')

write_csv(prs_scores_all, 'prs_no_social.csv')
write_csv(res_scores_all, 'res_reg_only.csv')
...
```{r soc_resilience}
# replace resilience_matrix.csv with resilience_matrix_soc.csv
file.copy(from = 'conf/resilience_matrix_soc.csv',
          to   ='conf/resilience_matrix.csv',
          overwrite = TRUE)
...
write_csv(res_scores_all, 'res_soc_only.csv')
...
# replace resilience_matrix.csv with resilience_matrix_ecol.csv
file.copy(from = 'conf/resilience_matrix_ecol.csv',
          to   ='conf/resilience_matrix.csv',
          overwrite = TRUE)
...
write_csv(res_scores_all, 'res_ecol_only.csv')
```
